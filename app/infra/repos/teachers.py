...
class TeachersRepo:
    ...
    def upsert(self, teacher: Teacher) -> None:
        with self._connect(self._db_path) as cn:
            cn.execute(
                """
                INSERT INTO teachers(id, name, service_number, default_rate)
                VALUES(?,?,?,?)
                ON CONFLICT(id) DO UPDATE SET name=excluded.name, default_rate=excluded.default_rate
                """,
                (teacher.id, teacher.name,teacher.service_number, teacher.default_rate),
            )

    def get(self, teacher_id: int) -> Optional[Teacher]:
        with self._connect(self._db_path) as cn:
            row = cn.execute(
                "SELECT id, name, service_number, default_rate FROM teachers WHERE id=?", (teacher_id,)
            ).fetchone()
            if not row:
                return None
            return Teacher(
                id=int(row["id"]), name=row["name"], service_number = str(row["service_number"]), default_rate=float(row["default_rate"])
            )

    def list_all(self) -> List[Teacher]:
        with self._connect(self._db_path) as cn:
            return [
                Teacher(id=int(r["id"]), name=r["name"], service_number=str(r["service_number"]), default_rate=float(r["default_rate"]))
                for r in cn.execute(
                    "SELECT id, name, service_number, default_rate FROM teachers ORDER BY name"
                ).fetchall()
            ]
